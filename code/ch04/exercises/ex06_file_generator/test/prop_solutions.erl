...
prop_make_tmp_file() ->
    ...
    % Since the folder system was created ([tmp/Cache/])when the temporary file was created,
    % it would be correct to delete them as well.
    % !!! Be careful! Do not accidentally delete anything you need and important.
	%
    %Get temp file directory
    %Dir = filename:dirname(TmpFilePath),
    %io:format("Temp File Dir path: ~p~n", [Dir]),
    %ok = file:del_dir_r(Dir),
    %MainTmpDir = filename:dirname(Dir),
	%io:format("Main temp Dir path: ~p~n", [MainTmpDir]),
